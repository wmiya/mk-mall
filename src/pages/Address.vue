<template>
  <div>
    ...
    <modal :mdShow="isMdShow2"
           @close="isMdShow2=false">
      <template v-slot:message>
...
export default {
  ...
  computed: {
    ...
    selectedAddrId () {
      return this.addressList.filter(item => item.isDefault)
    }
  },
  mounted () {
    this.init()
  ...
  methods: {
    async  init () {
      let { data: { status, result } } = await this.axios.get('/users/addressList')
      if (status === 0) {
        result.forEach((item, index) => {
          if (item.isDefault == true) {
            result.splice(index, 1, ...result.splice(0, 1, item))
          }
        });
    ...
    async  setDefault (addressId) {
      let { data: { status, result } } = await this.axios.post('/users/setDefault', {
        addressId: addressId
      })
      if (status === 0) {
        this.init()
    ...
    async  delAddress () {
      // 删除接口
      let { data: { status, result } } = await this.axios.post('/users/delAddress', {
