<template>
  <div>
    ...
    <modal v-bind:mdShow="modalConfirm"
...
export default {
  ...
  methods: {
    ...
    async editCart (type, item) {
      ...
      let { data: { status, result } } = await this.axios.post('/users/cartEdit', {
    ...
    async  toggleCheckAll () {
      ...
      let { data: { status, result } } = await this.axios.post('/users/editCheckAll', {
        checkAll: flag ? '1' : '0'
      })
      if (status === 0) {
